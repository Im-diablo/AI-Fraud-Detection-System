...
# Dynamically install gradio on Hugging Face if it's missing (keeps local requirements.txt clean)
try:
    import gradio as gr
except ImportError:
...
# ── Gradio integration for Hugging Face Spaces SDK compatibility ─────────────
demo = gr.Interface(
    fn=lambda x: x,
    inputs=gr.Textbox(placeholder="Gradio interface placeholder..."),
    outputs="text",
    title="AI Fraud Detection Backend",
)
app = gr.mount_gradio_app(app, demo, path="/gradio")

# ── Hugging Face Space runner entry point ─────────────────────────────────────
# We run the Uvicorn server directly if we are in the Hugging Face environment.
# We use the UVICORN_RUNNING env guard to prevent a recursive import loop.
if "UVICORN_RUNNING" not in os.environ:
    os.environ["UVICORN_RUNNING"] = "1"
    import uvicorn
    port = 7860 if "SPACE_ID" in os.environ else 8000
    uvicorn.run("app_hf:app", host="0.0.0.0", port=port, reload=False)
